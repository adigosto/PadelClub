...
import 'package:padelclub_desktop/features/order_items/data/models/order_item_model.dart';
import 'package:padelclub_desktop/providers/auth_provider.dart';

abstract class OrderItemRemoteDataSource {
  Future<List<OrderItemModel>> getOrderItems({Map<String, dynamic>? filter});
}
...
class OrderItemRemoteDataSourceImpl implements OrderItemRemoteDataSource {
  ...
  OrderItemRemoteDataSourceImpl({required this.client, this.baseUrl = const String.fromEnvironment('baseUrl', defaultValue: 'http://localhost:5001/api')});

  @override
  Future<List<OrderItemModel>> getOrderItems({Map<String, dynamic>? filter}) async {
    var url = '$baseUrl/OrderItems';
