...
class ReservationProvider extends ChangeNotifier {
  ...
  ReservationProvider({String? baseUrl}) {
    _baseUrl = baseUrl ?? const String.fromEnvironment('_baseUrl', defaultValue: 'http://localhost:5001/api');
  }

  Future<List<Reservation>> get({Map<String, dynamic>? filter}) async {
    var url = '$_baseUrl/Reservations';
    ...
    if (isValidResponse(response)) {
      ...
      final items = data.map((e) => ReservationModel.fromJson(e as Map<String, dynamic>)).toList();
      return items.map((m) => Reservation(id: m.id, courtId: m.courtId, userId: m.userId, start: m.start, end: m.end)).toList();
    } else {
