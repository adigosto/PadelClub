...
class TournamentProvider extends ChangeNotifier {
  ...
  TournamentProvider({String? baseUrl}) {
    _baseUrl = baseUrl ?? const String.fromEnvironment('_baseUrl', defaultValue: 'http://localhost:5001/api');
  }

  Future<List<Tournament>> get({Map<String, dynamic>? filter}) async {
    var url = '$_baseUrl/Tournament';
    ...
    if (isValidResponse(response)) {
      ...
      return items.map((m) => Tournament(id: m.id, name: m.name)).toList();
    } else {
